...
/**
 * SOTA 2026 Granular Error Boundary: Geopolitical Map (Elite)
 *
 * Target: D3.js/MapCanvas crashes.
 * Features:
 * - Domain-specific OTel tagging
 * - Durable State Recovery (coord-restore potential)
 */

import { motion } from "framer-motion";
import { Activity, RefreshCcw, ShieldX } from "lucide-react";
...
export default function MapError({
	...
	useEffect(() => {
		// OTel Capture - Specific Tagging for Geopolitical domain
		console.error("GEOPOLITICAL_MAP_CORE_FAULT", {
			severity: "HIGH",
			domain: "Geopolitical",
			subdomain: "D3MapCanvas",
			digest: error.digest,
			message: error.message,
			isRecoverable: true,
			timestamp: new Date().toISOString(),
			tags: ["geopolitical-map", "d3-v8", "webgl-context-lost"],
		});
	}, [error]);

	const handleReset = () => {
		// Durable State Recovery: In 2026, we might try to revert to a
		// "safe" projection or clear the latest invalid marker from local storage.
		if (typeof window !== "undefined") {
			console.warn("Attempting Geopolitical Domain Recovery...");
		}
		startTransition(() => {
			reset();
		});
				...
				<Card className="bg-zinc-950/40 border-destructive/20 rounded-[3rem] shadow-[0_30px_60px_-15px_rgba(220,38,38,0.2)] overflow-hidden relative backdrop-blur-3xl">
					...
					<CardHeader className="flex flex-col items-center text-center pt-14 pb-8">
						<div className="h-24 w-24 bg-destructive/5 rounded-[2rem] flex items-center justify-center mb-8 border border-destructive/10 relative overflow-hidden group">
							...
							<ShieldX className="h-10 w-10 text-destructive relative z-10" strokeWidth={1} />
						</div>
						...
						<CardDescription className="max-w-[320px] text-zinc-500 text-sm leading-relaxed">
							A catastrophic coordinate transform error occurred. The viewport state has been locked
					...
					<CardContent className="px-12 pb-14 flex flex-col items-center">
						<div className="w-full bg-white/[0.02] rounded-2xl p-6 mb-10 border border-white/5 flex items-start gap-4 shadow-inner">
							...
							<div className="flex flex-col gap-1">
								...
								<code className="text-[11px] font-mono text-zinc-400 break-all leading-relaxed">
									{error.message || "Transform fault in GeoMapCore: Projection Overflow"}
								</code>
							</div>
						</div>
						...
						<div className="flex flex-col w-full gap-4">
							...
							>
								...
								{isPending ? "Recalculating Projections..." : "Reset Map Engine"}
							</Button>
