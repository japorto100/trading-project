...
import type { Candle, ChartConfig, CrosshairState, Drawing, Indicator, Viewport } from "../types";
...
export class ChartRenderer {
	...
	renderCrosshair(crosshair: CrosshairState | null, viewport: Viewport) {
		...
		ctx.fillRect(
			viewport.width - priceLabelWidth,
			crosshair.y - priceLabelHeight / 2,
			priceLabelWidth,
			priceLabelHeight,
		);
		...
		ctx.fillStyle = this.config.crosshairColor;
		ctx.fillRect(
			crosshair.x - timeLabelWidth / 2,
			viewport.height - timeLabelHeight,
			timeLabelWidth,
			timeLabelHeight,
		);
	...
	private renderGrid(viewport: Viewport) {
		...
		for (
			let price = Math.floor(priceRange.min / priceInterval) * priceInterval;
	...
	private renderCandles(viewport: Viewport, _candles: Candle[]) {
	...
	private renderVolume(viewport: Viewport, _candles: Candle[]) {
	...
	private renderDrawings(viewport: Viewport, drawings: Drawing[]) {
		...
		drawings.forEach((drawing) => {
			...
			ctx.setLineDash(
				drawing.lineStyle === "dashed" ? [5, 5] : drawing.lineStyle === "dotted" ? [2, 2] : [],
			);
	...
	private renderFibonacci(ctx: CanvasRenderingContext2D, drawing: Drawing, viewport: Viewport) {
		...
		levels.forEach((level) => {
			...
			ctx.fillStyle = drawing.color;
			ctx.fillText(
				`${(level * 100).toFixed(1)}% - ${this.coord!.yToPrice(y).toFixed(2)}`,
				Math.min(x1, x2) + 5,
				y - 3,
			);
		});
	...
	private calculatePriceGridInterval(_viewport: Viewport): number {
