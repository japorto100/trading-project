...
import { Layer, ChartState, Candle, Viewport, ChartConfig, Drawing, IndicatorConfig, IndicatorResult } from './types';
...
export class GridLayer implements Layer {
  ...
  private calculateTimeSteps(candles: Candle[], viewport: Viewport, width: number): number[] {
    const { viewStartIndex, viewEndIndex, pxPerCandle } = viewport;
    const totalCandleWidth = this.config.candleWidth + this.config.candleSpacing;
...
export class CrosshairLayer implements Layer {
  ...
  render(ctx: CanvasRenderingContext2D, state: ChartState): void {
    ...
    ctx.fillRect(priceScaleBounds.left, mouseY - 12, priceScaleBounds.right - priceScaleBounds.left, 24);
    ...
    if (time !== null) {
      const timeframeMs = viewport.pxPerCandle > 50 ? 60000 : viewport.pxPerCandle > 20 ? 3600000 : 86400000;
      const timeLabel = this.coordSystem.formatTime(time, timeframeMs);
      
      ctx.fillStyle = this.config.crosshairLabelBg;
      ctx.fillRect(mouseX - 40, timeScaleBounds.top, 80, timeScaleBounds.bottom - timeScaleBounds.top);
...
export class DrawingLayer implements Layer {
  ...
  private renderTrendline(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
  ...
  private renderHorizontalLine(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
  ...
  private renderVerticalLine(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
    if (drawing.points.length < 1) return;
  ...
  private renderRectangle(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
    if (drawing.points.length < 2) return;
  ...
  private renderFibonacciRetracement(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
    if (drawing.points.length < 2) return;
    ...
    for (const level of fibLevels) {
      ...
      ctx.textAlign = 'right';
      ctx.fillText(`${(level * 100).toFixed(1)}% - ${this.coordSystem.formatPrice(price)}`, mainBounds.right - 5, y - 3);
    }
  }
  ...
  private renderMeasure(ctx: CanvasRenderingContext2D, drawing: Drawing, state: ChartState): void {
    ...
    const bars = Math.abs(candles.findIndex(c => c.time === p2.time) - candles.findIndex(c => c.time === p1.time));
    ...
    ctx.font = '11px monospace';
    ctx.textAlign = 'center';
    ctx.fillText(`${priceDiff >= 0 ? '+' : ''}${this.coordSystem.formatPrice(priceDiff)}`, midX, midY - 8);
...
export class PriceScaleLayer implements Layer {
  ...
  render(ctx: CanvasRenderingContext2D, state: ChartState): void {
    ...
    // Background
    ctx.fillStyle = this.config.backgroundColor;
    ctx.fillRect(priceScaleBounds.left, priceScaleBounds.top, priceScaleBounds.right - priceScaleBounds.left, priceScaleBounds.bottom - priceScaleBounds.top);
...
export class TimeScaleLayer implements Layer {
  ...
  render(ctx: CanvasRenderingContext2D, state: ChartState): void {
    ...
    // Background
    ctx.fillStyle = this.config.backgroundColor;
    ctx.fillRect(timeScaleBounds.left, timeScaleBounds.top, timeScaleBounds.right - timeScaleBounds.left, timeScaleBounds.bottom - timeScaleBounds.top);
  ...
  private estimateTimeframe(candles: Candle[], viewport: Viewport): number {
