...
export class ChartEngine {
  ...
  constructor(options: ChartEngineOptions) {
    ...
    this.coordSystem = new CoordinateSystem(this.config);
  ...
  private handleMouseMove(e: MouseEvent): void {
    ...
    if (this.activeDrawingTool && this.tempDrawingPoints.length > 0) {
      ...
      const price = this.coordSystem.yToPrice(this.mouseY, this.viewport, this.canvas.height / this.dpr, this.scaleType);
    ...
    if (this.mouseX !== null && this.mouseY !== null) {
      const time = this.coordSystem.xToTime(this.mouseX, this.candles, this.viewport);
      const price = this.coordSystem.yToPrice(this.mouseY, this.viewport, this.canvas.height / this.dpr, this.scaleType);
      const candleIndex = Math.floor(this.viewport.viewStartIndex + (this.mouseX - this.config.padding.left) / this.viewport.pxPerCandle);
  ...
  private handleMouseDown(e: MouseEvent): void {
    ...
    if (this.activeDrawingTool) {
      ...
      const price = this.coordSystem.yToPrice(y, this.viewport, this.canvas.height / this.dpr, this.scaleType);
  ...
  private handleWheel(e: WheelEvent): void {
    ...
    const centerIndex = this.viewport.viewStartIndex + (mouseX - this.config.padding.left) / this.viewport.pxPerCandle;
    ...
    const chartWidth = this.canvas.width / this.dpr - this.config.padding.left - this.config.padding.right - this.config.priceScaleWidth;
  ...
  private calculateInitialViewport(): void {
    ...
    const chartWidth = width - this.config.padding.left - this.config.padding.right - this.config.priceScaleWidth;
  ...
  setIndicatorData(id: string, data: any[]): void {
  ...
  goTo(time: number): void {
    ...
    this.viewport.viewEndIndex = Math.min(this.candles.length, this.viewport.viewStartIndex + visibleCount);
