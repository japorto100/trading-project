...
const severityValues = [1, 2, 3, 4, 5] as const satisfies readonly GeoSeverity[];
const statusValues: readonly GeoEventStatus[] = [
	"candidate",
	"confirmed",
	"persistent",
	"archived",
...
export function MarkerListModal({
	...
	const [minSeverityValue, setMinSeverityValue] = useState<GeoSeverity | 0>(0);
	const [statusValue, setStatusValue] = useState<GeoEventStatus | "all">("all");
	...
	const filteredEvents = useMemo(() => {
		...
		return events.filter((event) => {
			if (minSeverityValue > 0 && event.severity < minSeverityValue) return false;
			if (statusValue !== "all" && event.status !== statusValue) return false;
			if (country.length > 0) {
				const hasCountry = event.countryCodes.some((entry) =>
					entry.toLowerCase().includes(country),
				);
			...
			if (symbol.length > 0 && !event.symbol.toLowerCase().includes(symbol)) return false;
