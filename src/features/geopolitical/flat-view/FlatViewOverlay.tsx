...
import type { GeoFlatViewOverlayEventPoint } from "@/features/geopolitical/flat-view/flat-view-overlay-payload";
	...
	const layers = useMemo(
		() => [
			...
			new ScatterplotLayer<GeoFlatViewOverlayEventPoint>({
				id: "flat-handoff-event-halos",
				data: eventsLayerEnabled
					? contract.eventPoints.filter(
							(point) => point.declutterVisible && (point.badgeVisible || point.labelVisible),
						)
					: [],
				pickable: false,
				stroked: false,
				...
				getRadius: (point) => point.haloRadiusMeters,
				radiusUnits: "meters",
			...
			new ScatterplotLayer<GeoFlatViewOverlayEventPoint>({
				...
				data: eventsLayerEnabled
					? contract.eventPoints.filter((point) => point.declutterVisible)
					: [],
				pickable: true,
				autoHighlight: true,
				stroked: true,
			...
			new TextLayer<GeoFlatViewOverlayEventPoint>({
				id: "flat-handoff-event-badges",
				data: eventsLayerEnabled
					? contract.eventPoints.filter((point) => point.declutterVisible && point.badgeVisible)
					: [],
				pickable: false,
				getPosition: (point) => point.coordinates,
				getText: (point) => point.symbolShortCode,
				getSize: (point) => (point.selected ? 12 : point.priorityTier === "critical" ? 11 : 10),
				getColor: [6, 10, 20, 255],
			...
			new TextLayer<GeoFlatViewOverlayEventPoint>({
				id: "flat-handoff-event-labels",
				data: eventsLayerEnabled
					? contract.eventPoints.filter((point) => point.declutterVisible && point.labelVisible)
					: [],
				pickable: false,
				getPosition: (point) => point.coordinates,
				getText: (point) => point.labelText,
				getSize: (point) => (point.selected ? 16 : point.priorityTier === "critical" ? 14 : 12),
				getColor: (point) => (point.selected ? [255, 248, 196, 255] : [226, 232, 240, 235]),
				getPixelOffset: [0, -18],
				getTextAnchor: "middle",
				getAlignmentBaseline: "bottom",
				getBackgroundColor: [8, 17, 31, 180],
				background: true,
				billboard: true,
			}),
		],
