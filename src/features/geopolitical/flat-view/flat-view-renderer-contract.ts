...
} from "@/features/geopolitical/flat-view/flat-view-layer-matrix";
import {
...
} from "@/features/geopolitical/flat-view/flat-view-timeline-model";
import {
...
export function buildGeoFlatViewRendererContract(params: {
	...
	const timelineModel = buildGeoFlatViewTimelineModel({
	...
	const selectedTimelineEventIds =
		timelineModel.selectedBucketIndex !== null && timelineModel.selectedBucketIndex >= 0
			? new Set(timelineModel.buckets[timelineModel.selectedBucketIndex]?.eventIds ?? [])
			: undefined;
