...
import type { AlertSeverityThreshold } from "@/features/geopolitical/phase12/types";
import type { GeoEvent } from "@/lib/geopolitical/types";

interface Phase12AlertsSectionProps {
	policyLoading: boolean;
	alertSeverityThreshold: AlertSeverityThreshold;
	setAlertSeverityThreshold: (value: AlertSeverityThreshold) => void;
...
export function Phase12AlertsSection(props: Phase12AlertsSectionProps) {
	return (
		<section className="rounded border border-border bg-background p-2">
			...
			<div className="grid gap-2 sm:grid-cols-2">
				<label className="text-xs">
					...
					<select
						id="phase12-alert-min-severity"
						name="phase12_alert_min_severity"
						className="w-full rounded border border-border bg-background px-2 py-1"
						value={props.alertSeverityThreshold}
						onChange={(event) =>
							props.setAlertSeverityThreshold(event.target.value as AlertSeverityThreshold)
						}
					>
				...
				<label className="text-xs">
					...
					<input
						id="phase12-alert-min-confidence"
						name="phase12_alert_min_confidence"
						type="range"
						min={0.4}
						max={0.99}
						step={0.01}
				...
				<label className="text-xs">
					...
					<input
						id="phase12-alert-cooldown-minutes"
						name="phase12_alert_cooldown_minutes"
						type="range"
						min={5}
						max={240}
						step={5}
				...
				<div className="space-y-1 text-xs">
					<label className="flex items-center gap-2">
						<input
							id="phase12-alert-mute-profile"
							name="phase12_alert_mute_profile"
							type="checkbox"
					...
					<label className="flex items-center gap-2">
						<input
							id="phase12-alert-playback-tie-in"
							name="phase12_alert_playback_tie_in"
							type="checkbox"
