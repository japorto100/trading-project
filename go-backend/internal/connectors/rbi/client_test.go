...
func TestGetSeries_FetchesDBIEFXReservesAndParsesLatestFirst(t *testing.T) {
	...
	points, err := client.GetSeries(context.Background(), currency.NewPair(currency.NewCode("RBI_DBIE_FXRES_TR_USD_WEEKLY"), currency.NewCode("USD")), asset.Empty, 2)
	if err != nil {
...
func TestGetTicker_UsesCanonicalCurrency(t *testing.T) {
	...
	client := NewClient(Config{BaseURL: server.URL})
	ticker, err := client.GetTicker(context.Background(), currency.NewPair(currency.NewCode("FXRES_TR_USD_WEEKLY"), currency.NewCode("USD")), asset.Empty)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestGetSeries_RejectsInvalidSeries(t *testing.T) {
	...
	_, err := client.GetSeries(context.Background(), currency.NewPair(currency.NewCode("RBI_DBIE_BAD!"), currency.NewCode("USD")), asset.Empty, 1)
	if err == nil {
