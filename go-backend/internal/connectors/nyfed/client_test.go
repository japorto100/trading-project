...
func TestGetSeries_FetchesAndParses(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		gotEndDate = r.URL.Query().Get("endDate")
		_, _ = w.Write([]byte(`{
			"refRates":[
				{"effectiveDate":"2026-03-03","type":"SOFR","percentRate":3.70},
				{"effectiveDate":"2026-03-04","type":"SOFR","percentRate":3.65}
			]
		}`))
	}))
	defer server.Close()
	...
	points, err := client.GetSeries(
		context.Background(),
		currency.NewPair(currency.NewCode("NYFED_SOFR"), currency.NewCode("USD")),
		assetItem,
		10,
	)
	if err != nil {
	...
	if gotPath != "/rates/secured/sofr/search.json" {
		t.Fatalf("unexpected path %q", gotPath)
	}
	if gotStartDate == "" || gotEndDate == "" {
		t.Fatalf("expected populated date range, got start=%q end=%q", gotStartDate, gotEndDate)
	}
	if len(points) != 2 {
...
func TestGetSeries_InvalidSymbolReturnsBadRequest(t *testing.T) {
	...
	_, err := client.GetSeries(
		context.Background(),
		currency.NewPair(currency.NewCode("NYFED_UNKNOWN"), currency.NewCode("USD")),
		assetItem,
		10,
	)
	if err == nil {
