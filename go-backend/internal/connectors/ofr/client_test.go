...
func TestGetSeries_UsesHotCache(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		callCount++
		_, _ = w.Write([]byte(`{"REPO-TRIV1_AR_OO-P":{"timeseries":{"aggregation":[["2026-03-04",3.65]]}}}`))
	}))
	defer server.Close()
	...
	assetItem, _ := asset.New("macro")
	for range 2 {
		points, err := client.GetSeries(context.Background(), currency.NewPair(currency.NewCode("OFR_REPO-TRIV1_AR_OO-P"), currency.NewCode("USD")), assetItem, 1)
		if err != nil {
			t.Fatalf("GetSeries: %v", err)
		}
		if len(points) != 1 {
