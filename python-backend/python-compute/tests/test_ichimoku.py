...
# ---------------------------------------------------------------------------
# Chikou signals (chikou_above_cloud / chikou_below_cloud)
# ---------------------------------------------------------------------------

def test_chikou_signals_length_matches_n():
    n = 100
    req = IchimokuRequest(ohlcv=_ohlcv(n), include_future=False)
    r = calculate_ichimoku(req)
    assert len(r.signals.chikou_above_cloud) == n
    assert len(r.signals.chikou_below_cloud) == n


def test_chikou_signals_mutually_exclusive():
    """chikou_above_cloud and chikou_below_cloud cannot both be True at the same bar."""
    req = IchimokuRequest(ohlcv=_ohlcv(100), include_future=False)
    r = calculate_ichimoku(req)
    for i in range(len(r.signals.chikou_above_cloud)):
        assert not (r.signals.chikou_above_cloud[i] and r.signals.chikou_below_cloud[i]), \
            f"Bar {i}: both chikou_above_cloud and chikou_below_cloud are True"


def test_chikou_early_bars_both_false():
    """Bars with i < displacement have no historical cloud reference → both must be False."""
    d = 26
    req = IchimokuRequest(ohlcv=_ohlcv(100), displacement=d, include_future=False)
    r = calculate_ichimoku(req)
    for i in range(d):
        assert not r.signals.chikou_above_cloud[i], f"chikou_above_cloud[{i}] should be False (no cloud ref)"
        assert not r.signals.chikou_below_cloud[i], f"chikou_below_cloud[{i}] should be False (no cloud ref)"


def test_chikou_above_cloud_in_strong_uptrend():
    """Strong uptrend: closes[i] should be above the cloud from 26 bars ago for mature bars."""
    # 200 bars strongly trending up — chikou (= closes[i]) should be above old cloud
    pts = _ohlcv(200, start_close=100.0, step=2.0)
    req = IchimokuRequest(ohlcv=pts, displacement=26, include_future=False)
    r = calculate_ichimoku(req)
    # Check tail where cloud reference is well-established
    tail_above = r.signals.chikou_above_cloud[-20:]
    assert any(tail_above), "Expected chikou_above_cloud=True in strong uptrend tail"


def test_chikou_no_lookahead():
    """chikou signal uses closes[i] vs cloud at i-d — no future data."""
    # Verify: for bar i, chikou_above_cloud[i] == (closes[i] > cloud_top at i-d)
    d = 26
    pts = _ohlcv(100, step=1.0)
    req = IchimokuRequest(ohlcv=pts, displacement=d, include_future=False)
    r = calculate_ichimoku(req)
    # At bar d: cloud ref is span_a[0] and span_b[0]
    sa0 = r.span_a[0].value
    sb0 = r.span_b[0].value
    if not math.isnan(sa0) and not math.isnan(sb0):
        cloud_top = max(sa0, sb0)
        close_d = pts[d].close
        expected = close_d > cloud_top
        assert r.signals.chikou_above_cloud[d] == expected
