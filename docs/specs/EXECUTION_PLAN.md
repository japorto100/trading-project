...
> **Stand:** 09. März 2026 (Rev. 3.28 — Market Credential Flow Slice + Verify-Gates dokumentiert)
...
> - Rev. 3.27 (05. Mär 2026) — **Phase 1 ABGESCHLOSSEN. 1.v24 InactivityMonitor + Soft Lock:** `InactivityMonitor.tsx` (`useIdleTimer` 10min, crossTab BroadcastChannel, `react-idle-timer@5.7.2`), `LockScreen.tsx` (z-[9999] blur-Overlay, Re-Auth, 5-Strike Hard-Logout), `providers.tsx` (dynamic ssr:false), `auth.ts` `maxAge` 15min→8h. `bun run lint + build` clean.
> - Rev. 3.28 (09. Mär 2026) — **Market Credential Flow Slice erweitert.** Browser-/Frontend-Provider-Keys werden jetzt ueber `SettingsPanel` zusaetzlich in einen server-lesbaren Cookie synchronisiert (`tradeview_provider_credentials`); Next-Market-Routen (`quote`, `providers`, `stream`, `stream/quotes`) rekonstruieren daraus kontrolliert `X-Tradeview-Provider-Credentials`; Go-Gateway legt den Header request-scoped in `requestctx` ab; `internal/connectors/finnhub` nutzt request-scoped Credentials vor ENV-Fallback. Live-Verify-Gates fuer diesen Slice in `execution_mini_plan_3.md` dokumentiert.
...
> - Phase 0c/1d (vertical slice erweitert, 09. Mär 2026): Browserseitig gespeicherte Provider-Keys werden jetzt ueber `SettingsPanel` in einen server-lesbaren Cookie synchronisiert; Next-Market-Routen (`quote`, `providers`, `stream`, `stream/quotes`) nutzen denselben Credential-Pfad bis zum Go-Gateway. Go legt `X-Tradeview-Provider-Credentials` request-scoped in `requestctx` ab; `finnhub` ist der erste read-only Connector, der diese Credentials vor ENV-Fallback verwendet. Verify-Gates in `execution_mini_plan_3.md`.
...
> **Performance-Parameter Review bei Rust-Migration (05. März 2026):**
> Wenn Python-Berechnungen nach Rust migriert werden, müssen Simulation-Counts und Iterations-Parameter grundsätzlich neu bewertet werden — Rust ist typischerweise 10–100× schneller als Python für numerische Workloads. Konkrete Review-Pflicht bei Migration folgender Komponenten:
> - **Monte Carlo VaR** (Phase 13, `MonteCarloVarPanel`): IST `simulations=10_000` — SOLL nach Rust-Migration: `≥100_000`, ggf. als konfigurierbarer Parameter
> - **Monte Carlo Backtesting** (Phase 16): Walk-Forward + Triple-Barrier Sims — Anzahl der Pfade hochschrauben
> - **Game Theory Monte Carlo** (Phase 17, `/api/v1/game-theory/monte-carlo`): Szenarien-Zahl erhöhen
> - **Alle Python-Loops** mit fixem `limit=` oder `n_simulations=` Parameter: Vor Rust-Migration dokumentieren, nach Migration evaluieren ob Schwellenwert sinnvoll angepasst werden kann
> - **Faustregel:** Bei Python→Rust Migration für Simulations-/Iterations-Code immer prüfen ob die Zahl aus Performance-Gründen oder fachlichen Gründen gewählt wurde. Performance-bedingte Limits aggressiv erhöhen.
>
...
| **4c** | Canvas/SVG Hybrid | GEO Sek. 35.4, 35.4e | Canvas fuer statische Elemente (Basemap/Graticule/Heatmap), SVG fuer interaktive Marker/Drawings. Viewport-Culling. Supercluster. Body-agnostische Renderer-Hooks. **Optional:** OffscreenCanvas wenn PERFORMANCE_BASELINE Szenario B (<45 FPS); supercluster Worker bei 5k+ Events (GEO 35.4e, GEOPOLITICAL_OPTIONS Sek. 7.3). |
...
> **Optional Performance (Phase 4/12):** OffscreenCanvas (GeoMap Canvas → Worker) und supercluster Worker (5k+ Events) siehe [`GEOPOLITICAL_MAP_MASTERPLAN.md`](../../docs/GEOPOLITICAL_MAP_MASTERPLAN.md) Sek. 35.4e, [`PERFORMANCE_BASELINE.md`](../../docs/PERFORMANCE_BASELINE.md).
