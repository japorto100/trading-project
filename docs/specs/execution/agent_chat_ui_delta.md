...
### Abgeschlossen — Frontend Core (Phase A) + Rich-Text/Speech (Phase B)
...
| `hooks/useChatSession.ts` | Session-isolierter Reducer + SSE-Parser + abort |
...
- Go Gateway: `/api/v1/agent/chat` SSE-Endpoint (AC6)
- Python memory-service: Chat-Handler + Anthropic Streaming (AC7)
- Thread-Persistenz via `AgentEpisode` (AC10/AC11)
...
- [ ] **AC6** Go Gateway `/agent/chat` SSE-Endpoint — **BACKEND OFFEN**
- [ ] **AC7** Python memory-service Chat-Handler + Anthropic Streaming — **BACKEND OFFEN**
...
- [ ] **AC39** Inline-Citation-Rendering (`[1]`, `[2]` als klickbare Links — ausstehend)
...
- [ ] **AC50** TTS-State-Store (autoplay-Queue) — ausstehend; Exklusivitaet via `speakingId` lokal geloest

### K. File Attachments (NEU)

- [ ] **AC51** Paperclip-Button im Composer (zeigt Anzahl staged Dateien)
  - Referenz: Perplexica `MessageInputActions/Attach.tsx`
- [ ] **AC52** Staged-Attachment-Preview (Datei-Liste pre-send mit Remove-Button)
  - Referenz: AgentZero `attachmentsStore.js` + `inputPreview.html`
- [ ] **AC53** Drag & Drop + Clipboard-Paste fuer Attachments (inkl. Screenshots)
  - Referenz: AgentZero `dragDropOverlay.html`
  - SOTA-2026: Claude 3.5+ nimmt Bilder entgegen — Direkt-Paste von Chart-Screenshots
- [ ] **AC54** File-Display in Messages (Datei-Icon + Name + Groesse)
  - Referenz: Onyx `FileDisplay.tsx`
- [ ] **AC55** Image-Preview Modal (Klick auf Bild → Vollbild mit Zoom)
  - Referenz: Onyx `FullImageModal.tsx`
- [ ] **AC56** Multi-Modal Image Input (Base64-Encode + an Anthropic-Vision weiterreichen)
  - benoetigt: Backend (BFF muss Bild an Gateway weiterreichen)

### L. Sources & Citations Panel (NEU)

- [ ] **AC57** Sources-Panel unter assistant-Message (Source-Cards: Favicon + Titel + Domain)
  - "View N more" → Modal mit allen Quellen
  - Referenz: Perplexica `MessageSources.tsx`
  - **kein Backend noetig** (Backend liefert Sources im SSE-Event)
- [ ] **AC58** Source-Tag-Button (Quellen-Count-Badge → togglet Document-Sidebar)
  - Referenz: Onyx `SourceTag` + Sidebar-Integration
...
- [ ] **AC63** Structured-Output-Renderer (Agent gibt JSON zurueck → Frontend rendert als Tabelle/Card)
  - SOTA-2026: Anthropic/OpenAI JSON-Mode
  - Referenz: Onyx `CustomToolRenderer.tsx`
- [ ] **AC64** Context-Window-Pressure-Bar (sichtbar wenn Kontext voll wird)
  - SOTA-2026: Zeigt Nutzung relativ zu Limit; Warnung bei >80%
  - Degradation-Flag: `NO_CONTEXT_SPACE`
  - Referenz: Onyx implizit (compaction toasts)
...
- [ ] **AC66** Agent-Tool-Approval-Flow im Chat (vor kritischen Tool-Calls muss User approven)
  - Inline-Approval-Card im Thread: "Agent moechte [tool] ausfuehren" + Approve/Reject
  - Aligned mit Control-Slice AC18 (approval-write)
  - benoetigt: Backend (Gateway haelt Tool-Call pending bis Frontend antwortet)
...
- [ ] **AC.V25** Speech: kein Backend-Aufruf fuer STT/TTS (rein browser-seitig)
...
> **Status:** **PAUSIERT nach Analyse** (CLI-Agent hat hier gestoppt).
> **Entscheid:** Vercel AI SDK v5/v6 (`@ai-sdk/react`) bleibt Zielbild fuer Frontend-Layer.
> **TanStack AI:** Abgelehnt — Alpha 0.3.x (Maerz 2026), kein stabiles Release.
> **Eigener Transport (IST):** Bleibt als Fallback-Referenz; Go Gateway muss Protocol migrieren.
...
**ACs Phase 22d:**
- [ ] **AC78** `bun add ai @ai-sdk/react @ai-sdk/anthropic` — Pakete installieren
- [ ] **AC79** Go Gateway: `x-vercel-ai-ui-message-stream: v1` Header + typisiertes Event-Format
- [ ] **AC80** `useChatSession.ts` ersetzen durch `useChat` aus `@ai-sdk/react`
- [ ] **AC81** BFF `/api/agent/chat/route.ts` verschlanken — nur Auth-Header + Gateway-Proxy
- [ ] **AC82** `AgentChatMessage.tsx` auf `message.parts` umstellen (text / reasoning / tool)
- [ ] **AC83** `AgentChatMarkdown.tsx` bleibt — wird an `part.text` statt `block.content` uebergeben
- [ ] **AC84** Extended Thinking via `reasoning` Parts — ThinkBlock direkt aus SDK-Events (ersetzt `<think>` Tag-Parsing)
- [ ] **AC85** Tool-Call 4-State UI via SDK `tool-invocation` Part — ersetzt manuelles `tool_call/tool_result` Pair
- [ ] **AC86** `ChatBlock / ChatMessage / SseChunkEvent` Types bereinigen — nur noch SDK-kompatible Types

**Verify-Gates Phase 22d:**
- [ ] **AC.V36** `useChat` sendet + empfaengt SSE korrekt ueber Go Gateway
- [ ] **AC.V37** Extended Thinking Block erscheint via `reasoning` Part (kein `<think>` Tag noetig)
- [ ] **AC.V38** Tool-Call zeigt alle 4 States (input-streaming / input-available / output-available / output-error)
- [ ] **AC.V39** tsc 0 Fehler, Biome 0 Fehler nach Migration
- [ ] **AC.V40** Kein Regression in bestehenden Chat-Features (Markdown, TTS, Voice, Copy, Suggestion-Chips)
...
- [ ] **AC88** Proaktiver Badge — Chat-Icon im GlobalTopBar zeigt Puls-Badge wenn: (a) signifikante Marktbewegung via Stream-Event detektiert, (b) neues Geopolitik-Event hinzugefuegt; Badge-Count + dismiss on open
- [ ] **AC89** Split-View Modus — zwei Modi am gleichen Trigger: `sheet` (Overlay von rechts, Chart bleibt voll sichtbar) und `split` (Panel pushed Content-Bereich, 60/40 Aufteilung); Toggle im Chat-Header; default `sheet`
- [ ] **AC90** "Ask AI about this" Kontextmenue — Rechtsklick auf Candle / GeoEvent-Marker / offene Position → Kontextmenue-Eintrag "Ask AI" → Chat oeffnet sich pre-filled mit Element-Daten als Kontext-String
...
- [ ] **AC93** AI Copilot Rail — optionale duenne vertikale Leiste (240px) am rechten Rand; persistent sichtbar ohne Overlay-Modus; Power-User-Toggle; kein Remount bei Seitennavigation
- [ ] **AC94** Context-Bar im Chat-Header — zeigt aktiv injizierten Kontext als dismissible Chip ("BTC/USD · 1H · $95,420") direkt im Chat-Header; klickbar zum Bearbeiten
- [ ] **AC95** `useCompletion` fuer Indikator-Hover-Tooltips — single-shot AI-Erklaerung bei Hover auf RSI/BB/Elliott-Annotation; kein Thread-State; kommt nach Phase 22d (AI SDK Install); `filter={false}` in cmdk wenn Command Palette AI-Mode aktiv
