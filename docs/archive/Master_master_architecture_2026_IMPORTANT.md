...
> **Inhalt verteilt in:** `docs/specs/ARCHITECTURE.md`, `docs/specs/API_CONTRACTS.md`, `docs/specs/AUTH_SECURITY.md`, `docs/specs/EXECUTION_PLAN.md`, `docs/specs/DOCUMENTATION_ARCHITECTURE.md`, `docs/MEMORY_ARCHITECTURE.md`, `docs/CONTEXT_ENGINEERING.md`, `docs/AGENT_ARCHITECTURE.md`, `docs/AGENT_TOOLS.md`, `docs/CLAIM_VERIFICATION_ARCHITECTURE.md`, `docs/UNIFIED_INGESTION_LAYER.md`
> Dieses Dokument bleibt als historischer Architektur-Snapshot erhalten.
...
**Primärquellen:** `MRKTEDGE.AI-deep research chatgptp2.md`, `FRONTEND_ARCHITECTURE.md`, `EXECUTION_PLAN.md`, `AUTH_SECURITY.md`, `UNIFIED_INGESTION_LAYER.md`, `AGENT_ARCHITECTURE.md`, `AGENT_TOOLS.md`.
...
**Primärquellen:** `ARCHITECTURE.md`, `API_CONTRACTS.md`, `UNIFIED_INGESTION_LAYER.md`, `RUST_LANGUAGE_IMPLEMENTATION.md`, `execution/infra_provider_delta.md`.
...
**Primärquellen:** `ARCHITECTURE.md`, `API_CONTRACTS.md`, `UNIFIED_INGESTION_LAYER.md`, `RUST_LANGUAGE_IMPLEMENTATION.md`, `execution/infra_provider_delta.md`, `FRONTEND_ARCHITECTURE.md`.
...
**Aladdin-Gap (Event-Backbone):** Async Job-Queue, Replay, Fanout. OSS: NATS JetStream. Kafka wird explizit durch NATS ersetzt (kein Kafka). Priorität: Hoch. OSS-Prinzip: Alle Event-Komponenten Open Source.

**Primärquellen:** `ARCHITECTURE.md`, `EXECUTION_PLAN.md`, `API_CONTRACTS.md`, `execution/infra_provider_delta.md`.  
**Externe Validierung:** [WEB-14], [WEB-15].

### 8.6 Open-Source-Prinzip (Aladdin-Benchmark)

Scope: Retail/Prosumer, nicht Enterprise. Kein Snowflake, Kafka, eFront, Preqin, Azure-spezifische Dienste. Stattdessen: NATS JetStream, DuckDB/Apache Iceberg/Delta Lake.

**Soll-Zustand (Aladdin-like, nur OSS):**
...
| Data Lakehouse | DuckDB, Apache Iceberg, Delta Lake (Medallion) |
| Developer API | OpenAPI, openapi-generator. Priorität: Niedrig. |

**Was wir nicht übernehmen (Aladdin):**

| Aladdin-Feature | Nicht übernehmen (Begründung) |
|-----------------|-------------------------------|
| Kafka-Enterprise-Scale | NATS JetStream ersetzt Kafka |
...
- **`deck.gl` / WebGPU** bleiben mit expliziter Namensnennung erhalten, gehören aber in erster Linie zur GeoMap- und Performance-Entscheidung:
  ...
  - `GEOMAP_VERIFY.md` (Performance-Baseline Sek. 4)
  - `EXECUTION_PLAN.md`
  - `GEOMAP_FOUNDATION.md` (Rendering Foundation Sek. 4)
...
- **`Connect RPC`** bleibt mit Namensnennung erhalten, ist aber keine pauschale Browser-Standardentscheidung. Es gehört vor allem zu:
  - `execution/infra_provider_delta.md`
  - `GO_GATEWAY.md`
  - `ARCHITECTURE.md`
  - `EXECUTION_PLAN.md`
...
**Primärquellen:** `ERRORS.md`, `AUTH_SECURITY.md`, `ARCHITECTURE.md`, `API_CONTRACTS.md`, `ROLLOUT_GATES.md`.
...
`execution/infra_provider_delta.md` schärft den Infrastrukturpfad sinnvoll nach:
...
**Primärquellen:** `EXECUTION_PLAN.md`, `execution/cross_cutting_verify.md`, `execution/infra_provider_delta.md`, `execution/compute_delta.md`, `ARCHITECTURE.md`, `RUST_LANGUAGE_IMPLEMENTATION.md`.
...
- `GEOMAP_FOUNDATION.md` — Basemap, Geocoding, PMTiles, Rendering (konsolidiert)
...
- *(in GEOMAP_FOUNDATION)* — PMTiles-Verträge
...
- `execution/compute_delta.md` — ergänzender Mini-Plan / Zwischenzustand
- `execution/cross_cutting_verify.md` — zusätzliche Evolutions- und ToDo-Planung
- `execution/infra_provider_delta.md` — Infrastruktur-Evolution, Gemini-Vorschlag, Go→Rust→Python Schwerpunkt
...
- `PAYMENT_ADAPTER.md`
- `PLUGIN_PILOT.md`
- `execution/cross_cutting_verify.md`
- `execution/infra_provider_delta.md`
- `execution/compute_delta.md`
...
## Anhang F - Konsolidierungsaddendum (KG, Ontologie, Retrieval)

Dieses Addendum verdichtet die verbindlichen Kernregeln aus:

- `KG_MERGE_AND_OVERLAY_ARCHITECTURE.md`
- `KG_ONTOLOGY.md`
- `ai_retrieval_knowledge_infra_full.md`

### F.1 Namespace- und ID-Regeln (verbindlich)

| Namespace | Bedeutung | Beispiel |
|---|---|---|
| `g:` | global canonical | `g:company:lei:...` |
| `e:` | event / fast event layer | `e:geo:evt_...` |
| `u:` | user-local overlay object | `u:watchitem:...` |
| `c:` | claim / stance | `c:claim:...` |
| `x:` | evidence object | `x:evidence:...` |
| `s:` | simulation branch/object | `s:branch:...` |
| `m:` | semantic memory object/chunk | `m:chunk:...` |

Canonical-ID-Prioritaet:

1. LEI / FIGI / ISIN / Wikidata QID / formale IDs
2. deterministische interne IDs mit Typ-Praefix
3. keine user-generierten Zufalls-IDs fuer globale Wahrheiten

### F.2 Claim/Evidence/Stance (Pflichtmodell)

Subjektive Interpretation wird nicht als faktische KG-Kante gespeichert.
Stattdessen:

- Claims als explizite, zeitlich gueltige Aussagen
- Evidence als Quellen-/Daten-/Notiz-/Event-Belege
- Stance als relationale Einordnung (`supports`, `opposes`, `refines`, ...)

Normativ gilt:

- Meinung = Claim/Stance
- Fakt = canonical domain relation
- keine stille Ueberschreibung globaler Fakten durch User-Objekte

### F.3 Merge-Regeln (overlay-first, query-time merge)

Merge erfolgt auf Query-Ebene, nicht als physische Storage-Fusion.

- Global facts win auf Ontologie, canonical IDs und Domain-Fakten.
- User overlay wins auf Personalisierung (watchlist, positions, focus, annotations, private claims).
- Claims coexist with facts; Widerspruch wird modelliert, nicht ueberschrieben.
- Keine stillen Overwrites.

### F.4 Konfliktklassen (Kurzfassung)

- Typ A: Fakt vs. Meinung -> als Claim modellieren, kein Storage-Konflikt
- Typ B: alte vs. neue Evidenz -> Claim `challenged`, Evidence verlinken
- Typ C: Agent vs. User -> User-Vorrang im UI, Agent-Provenance erhalten
- Typ D: local vs. synced user data -> objektklassen-spezifische Sync-Policy
- Typ E: Simulation vs. Realitaet -> Simulation bleibt Branch-Artefakt

### F.5 Agent-Zugriffsregeln auf Wissensschichten

- Default: read-only auf global KG, overlay (falls freigegeben), semantic/episodic memory
- bounded-write: claim-vorschlaege, evidence-links, journals, simulation branches
- approval-write: produktiv wirksame Regeln/Actions und alle globalen Truth-Mutationen

### F.6 Simulation-Branch-Vertrag

Simulation ist Branching, nicht Wahrheit.

- `s:branch:*` basiert auf Event-/Claim-/Snapshot-Kontext
- persistierbar: branch metadata, outputs, summaries, extracted claims/evidence
- nicht persistierbar als canonical facts: hypothetische truths/states/edges

### F.7 Ontologiequellen (verbindlich)

Formale Schemaquellen:

- FIBO (Finance Ontology)
- Wikidata (Entitaeten/Properties/QIDs)
- GLEIF LEI (Company IDs)
- OpenFIGI (Instrument IDs)
- OpenSanctions (Sanktions-/PEP-Beziehungen)

Domaindokumente liefern Semantik, nicht das formale Schema.

### F.8 Retrieval-Rolle im Gesamtsystem

Retrieval ist eine eigene Architektur-Saeule:
...
- hybrid/federated retrieval statt Monokultur
- open web als discovery/freshness/gap-fill, nicht als direkte truth mutation
- graph fuer Struktur/Kausalitaet, vector fuer semantischen Recall